...
batch_size = 16
# num_neurons result from q3
num_neurons = 5
k_folds = 5

weight_decay_choices = [0, 1e-3, 1e-6, 1e-9, 1e-12]
...
for fold in range(k_folds):
    ...
    for i in weight_decay_choices:

        weight_decay = i

        print("Folds : ", fold_count , " Weight decay : " , weight_decay)
...
plt.figure(0)
for i in range(len(weight_decay_choices)):
    plt.plot(range(epochs), average_trainaccuracy[i])
plt.title('average train accuracy against epochs')
plt.ylabel('train accuracy')
plt.xlabel('epochs')
plt.legend(weight_decay_choices, loc='upper right')
plt.savefig('figures/q4_cvtrainaccuracy')
...
plt.xlabel('epochs')
plt.legend(weight_decay_choices, loc='upper right')
plt.savefig('figures/q4_cvtestaccuracy')
...
plt.plot(range(len(weight_decay_choices)), average_train_accuracy_per_epoch)
plt.plot(range(len(weight_decay_choices)), average_test_accuracy_per_epoch)
plt.xticks(range(len(weight_decay_choices)), [0, 1e-3, 1e-6, 1e-9, 1e-12])
plt.title('average train and test accuracies against weight decay')
plt.ylabel('average accuracy per epoch')
plt.xlabel('weight decay')
plt.legend(['train', 'test'], loc='upper right')
plt.savefig('figures/q4_averageaccuracyperepoch')
